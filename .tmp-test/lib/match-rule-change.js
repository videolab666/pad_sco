...
exports.matchHasStarted = matchHasStarted;
exports.isCurrentGameAtDeuce = isCurrentGameAtDeuce;
exports.isCurrentSetNearTiebreak = isCurrentSetNearTiebreak;
exports.isCurrentSetUnderway = isCurrentSetUnderway;
exports.isDecidingSetUnderway = isDecidingSetUnderway;
exports.classifyRuleChange = classifyRuleChange;
exports.reconcileSettingsForScoringSystem = reconcileSettingsForScoringSystem;
exports.normalizeCurrentGameForScoringSystem = normalizeCurrentGameForScoringSystem;
exports.normalizeCurrentSetForFinalSet = normalizeCurrentSetForFinalSet;
exports.normalizeMatchAfterRuleChange = normalizeMatchAfterRuleChange;
exports.backfillRuleMetadata = backfillRuleMetadata;
...
function isCurrentGameAtDeuce(currentGame) {
    if (!currentGame)
        return false;
    return (currentGame.teamA === "Ad" ||
        currentGame.teamB === "Ad" ||
        (currentGame.teamA === 40 && currentGame.teamB === 40));
}
/** True when the current set is at / past the tiebreak threshold. */
function isCurrentSetNearTiebreak(settings, currentSet) {
    if (!currentSet)
        return false;
...
function isCurrentSetUnderway(currentSet) {
    if (!currentSet)
        return false;
    const cg = currentSet.currentGame;
    return ((currentSet.teamA ?? 0) > 0 ||
...
function isDecidingSetUnderway(settings, score) {
    if (!score)
        return false;
    const completed = score.sets?.length ?? 0;
    const totalSets = Number(settings.sets) || 3;
    const onLastSet = completed + 1 >= totalSets;
    const setsToWin = (0, match_format_rules_1.getSetsToWin)(settings);
    const aSets = (score.sets ?? []).filter((s) => s.winner === "teamA").length;
    const bSets = (score.sets ?? []).filter((s) => s.winner === "teamB").length;
...
// ─── Classification ────────────────────────────────────────────────────────────
function diffKeys(oldS, newS) {
    return RULE_KEYS.filter((k) => {
        const a = oldS?.[k];
        const b = newS?.[k];
        if (k === "gamesPerSetOverrides")
            return JSON.stringify(a ?? {}) !== JSON.stringify(b ?? {});
        return a !== b;
    });
}
/** Classifies a single changed key against the live state. */
function classifyKey(key, oldS, newS, score) {
    ...
    switch (key) {
        ...
        case "sets": {
            ...
            if (newSets < played)
                return "restart-required";
            if (isDecidingSetUnderway(oldS, score) || isDecidingSetUnderway(newS, score))
                return "current-set";
 ...
 * offer a restart.
 */
function classifyRuleChange(oldSettings, newSettings, score) {
 ...
 */
function reconcileSettingsForScoringSystem(oldSettings, newSettings) {
 ...
 */
function normalizeCurrentGameForScoringSystem(currentGame, settings) {
    if (!currentGame)
        return;
 ...
 */
function normalizeCurrentSetForFinalSet(currentSet, settings) {
    if (!currentSet?.isTiebreak)
        return false;
    ...
    if (currentSet.isSuperTiebreak && !(0, match_format_rules_1.isMatchTiebreakFormat)(settings)) {
 ...
 */
function normalizeMatchAfterRuleChange(match, classification) {
 ...
 */
function backfillRuleMetadata(match) {
    if (!match || typeof match !== "object")
        return match;
    if (typeof match.ruleRevision !== "number")
        match.ruleRevision = 0;
    if (typeof match.revision !== "number")
        match.revision = 0;
    const settings = match.settings || (match.settings = {});
    if (settings.scoringSystem === undefined)
        settings.scoringSystem = "classic";
    if (settings.goldenPointFormat === undefined) {
        settings.goldenPointFormat = (0, match_format_rules_1.getDefaultGoldenPointForScoringSystem)(settings.scoringSystem);
    }
    if (settings.finalSetFinish === undefined)
        settings.finalSetFinish = "standard-7";
    if (settings.finalSetTiebreakLength === undefined)
        settings.finalSetTiebreakLength = 10;
    if (settings.tiebreakEnabled === undefined)
        settings.tiebreakEnabled = true;
    if (settings.tiebreakAt === undefined)
        settings.tiebreakAt = "6-6";
    if (settings.tiebreakLength === undefined)
        settings.tiebreakLength = 7;
    return match;
}
