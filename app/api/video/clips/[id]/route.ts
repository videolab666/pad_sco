// PATCH /api/video/clips/{id} — обновление статуса воркером/оператором.
// pending → processing → ready | failed; имена файлов без путей.

import { type NextRequest, NextResponse } from "next/server"
import { isAuthorizedSettingsRequest } from "@/lib/settings-auth"
import { updateClip } from "@/lib/clip-registry"
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const clip = await updateClip(id, {
