...
import { isGamePoint, isSetPoint, isMatchPoint, isBreakPoint, getBreakPointCount } from "@/lib/scoring-logic"
...
const buildVmixJson = (matchData: any, courtNumber: number) => ({
  id: matchData.id,
  courtNumber,
  teamA: {
...
export default function CourtVmixPage({ params }: CourtParams) {
  ...
  const { match, loading, error } = useVmixMatch({ kind: "court", courtNumber })
  ...
  const jsonOutput = match && outputFormat === "json" ? JSON.stringify(buildVmixJson(match, courtNumber), null, 2) : ""
  ...
  return (
    <>
      ...
      <VmixScoreboard match={match} settings={settings} variant="court" />
